...
const ConversationModal: React.FC<ModalProps> = ({ isOpen, onClose, session }) => {
  ...
  const [createConversations, { loading: createConversationLoading }] = useMutation<CreateConversationData, CreateConversationInput>(ConversationsOperations.Mutations.createConversation)
          ...
          <ModalBody>
            ...
            {participants.length !== 0 && (
              <>
                ...
                <Button isLoading={createConversationLoading} bg='brand.100' width='100%' mt={6} _hover={{ bg: 'brand.100' }} onClick={onCreateConversation}>Create Conversation</Button>
              </>
            )}
          </ModalBody>
        </ModalContent>
